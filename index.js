...
app.put('/updateMark/:rollNo', async (req, res) => {
  try {
    // Convert rollNo parameter to a number
    const rollNo = parseInt(req.params.rollNo);

    // Check if rollNo is a valid number
    if (isNaN(rollNo)) {
      return res.status(400).json({ message: 'Invalid Roll_No' });
    }

    // Extract marks from the request body
    const { WAD_Marks, CC_Marks, DSBDA_Marks, CNS_Marks, AI_Marks } = req.body;

    // Construct the update object with the new marks
    const update = {
      WAD_Marks: WAD_Marks,
      CC_Marks: CC_Marks,
    ...
    // Find and update the student document based on Roll_No
    const updatedStudent = await Student.findOneAndUpdate(
      { Roll_No: rollNo }, // Filter criteria
...
app.delete('/removeStudent/:rollNo', async (req, res) => {
  try {
    const { rollNo } = req.params;
    const removedStudent = await Student.findOneAndDelete({ Roll_No: rollNo });
